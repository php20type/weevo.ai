...
class UserController extends Controller
{
    ...
    {           
        request()->validate([
            'use-openai' => 'sometimes|required',
            'openai-key' => 'required_if:use-openai,on',

            'use-sd' => 'sometimes|required',
            'sd-key' => 'required_if:use-sd,on',
        ]);
        ...
        if (request('use-openai') == 'on') {
            $user->update([
                'enable_openai_api_usage' => true
            ]);
        } else {
            $user->update([
                'enable_openai_api_usage' => false
            ]);
        }

        if (request('use-sd') == 'on') {
            $user->update([
                'enable_sd_api_usage' => true
            ]);
        } else {
            $user->update([
                'enable_sd_api_usage' => false
            ]);
        }
